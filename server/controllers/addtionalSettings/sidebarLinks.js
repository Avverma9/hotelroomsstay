...
exports.addAllowedSidebarLinksToUser = async (req, res) => {
  try {
    const { userId } = req.params;
    const linkIds = normalizeIdList(req.body.linkIds || []);
    if (linkIds.length === 0) {
      return res.status(400).json({ message: "linkIds are required" });
    }

    const updatedUser = await DashboardUser.findByIdAndUpdate(
      userId,
      { $addToSet: { "sidebarPermissions.allowedLinkIds": { $each: linkIds } } },
      { new: true },
    )
      .select("role sidebarPermissions")
      .lean();
    ...
    return res.status(200).json({
      message: "Allowed sidebar links updated",
      data: updatedUser.sidebarPermissions,
    });
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
...
exports.addBlockedSidebarLinksToUser = async (req, res) => {
  try {
    const { userId } = req.params;
    const linkIds = normalizeIdList(req.body.linkIds || []);
    if (linkIds.length === 0) {
    ...
    const updatedUser = await DashboardUser.findByIdAndUpdate(
      userId,
      { $addToSet: { "sidebarPermissions.blockedLinkIds": { $each: linkIds } } },
      { new: true },
    )
      .select("role sidebarPermissions")
      .lean();
    ...
    return res.status(200).json({
      message: "Blocked sidebar links updated",
      data: updatedUser.sidebarPermissions,
    });
  } catch (error) {
    return res.status(500).json({ message: error.message });

...
const mockModel = {
  find: (filter) => {
    const docs = mockTours.filter(t => {
      if (filter && typeof filter.isAccepted !== 'undefined') return t.isAccepted === filter.isAccepted;
      return true;
    });
    const chain = {
      sort: () => chain,
  ...
  countDocuments: (filter) => Promise.resolve(mockTours.filter(t => t.isAccepted).length),
};
...
(async () => {
  try {
    console.log('Calling getAllTours with default query (no filters)...');

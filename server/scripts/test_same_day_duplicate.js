/**
 * Test script for Same-Day Duplicate Booking Logic
 * Run with: node server/scripts/test_same_day_duplicate.js
 */

// Mock bookingModel for testing
const mockActiveBookings = [
  {
    ...
    checkInDate: new Date("2024-03-15"), // 15 March
  ...
  },
  {
    ...
    checkInDate: new Date("2024-03-20"), // 20 March
    bookingStatus: "Confirmed"
  }
];

// Updated function with mock data parameter
async function checkDuplicateBooking(userId, userMobile, userEmail, currentHotelCity, currentHotelId, currentCheckInDate, mockData = null) {
  try {
    let activeBookings;
    
    if (mockData !== null) {
      // Use provided mock data for testing
      activeBookings = mockData;
    } else {
      // Use real database query (production) - but for this test, we'll return empty
      activeBookings = [];
    }
...
// Import the function (we'll manually inline it for testing)
async function checkDuplicateBooking(userId, userMobile, userEmail, currentHotelCity, currentHotelId, currentCheckInDate) {
  try {
    ...
    for (const booking of activeBookings) {
      ...
      if (isSameDay && currentCity === bookingCity) {
        // Same day + Same city
        if (currentHotel !== bookingHotel) {
          // Same day + Same city + Different hotel = Suspicious
          return {
            isDuplicate: true,
            reason: `Same-day booking detected: Active booking exists on ${bookingCheckIn.toDateString()} in ${booking.hotelDetails?.hotelCity || 'same city'} at different hotel`,
            shouldBePending: true
          };
        }
        // Same day + Same city + Same hotel = Re-booking (allow but check room details)
      } else if (currentCity === bookingCity && currentHotel !== bookingHotel) {
        // Different day + Same city + Different hotel = Standard duplicate check
        return {
          isDuplicate: true,
...
const testCases = [
  {
    ...
    reason: "Should be PENDING - Same day + same city + different hotel = suspicious"
  },
  {
  ...
  for (let i = 0; i < testCases.length; i++) {
    ...
    try {
      const result = await checkDuplicateBooking(
        ...
        test.mockData  // Pass mock data directly
      );
...
console.log("Second Booking Same Day + Same City = PENDING (any hotel)");  
console.log("Same Day + Different City = CONFIRMED (travel itinerary)");
console.log("Different Day + Same City + Different Hotel = PENDING (standard rule)");
